...
proc resname_log {} {
	...
	set outfile [open resname.tcl w]
	puts $outfile "#[pwd]"	
	echo "[pwd]"	
	...
	foreach resname_xx $resname_list {
		puts $outfile "set gemini_name $resname_xx"
	}
	close $outfile
	# vi resname.log
}
...
proc name_number { c_1 } {
	set resname_list [lsort -unique [[atomselect top all] get resname]]
	set outfile [open name_num.tcl w]
	puts $outfile "#[pwd]"	
	puts $outfile "#resname:$resname_list"
	...
	for {set i 0} { $i < $max_num } {incr i} {
		puts $outfile "set arrC([expr $i+1]) [lindex $c_1 $i]"
	}
	for {set i 0} { $i < $max_num } {incr i} {
...
proc update_mytcl {} {
	source /home/ryan/tcl/tclb-2015-1009.tcl
}
...
proc auto_mv_psf_dcd_2 {input_name} {
	cd $input_name
	echo "cd $input_name"
	echo "[pwd]"
	echo "auto_mv_psf_dcd"
	auto_mv_psf_dcd
	echo "done"
	cd ../
}
